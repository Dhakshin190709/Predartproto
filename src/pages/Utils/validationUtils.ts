...
const BASE_URL = 'https://predart003-001-site1.anytempurl.com/api/Login';
...
export const checkUsernameAvailability = async (username: string): Promise<{ success: boolean; message: string }> => {
  ...
  try {
    const response = await fetch(`${BASE_URL}/CheckUserNameExist?UserName=${trimmedName}`);
...
export const checkEmailAvailability = async (email: string): Promise<{ success: boolean; message: string }> => {
  ...
  try {
    const response = await axios.get(`${BASE_URL}/CheckEmailExist?Email=${trimmedEmail}`);
...
export const checkPhoneAvailability = async (phone: string): Promise<{ success: boolean; message: string }> => {
  const trimmedPhone = phone.trim();
  const phoneRegex = /^[0-9]{10}$/;

  if (!trimmedPhone) return { success: false, message: 'Phone number is required' };
  if (!phoneRegex.test(trimmedPhone)) return { success: false, message: 'Enter a valid 10-digit number' };

  try {
    const response = await fetch(`${BASE_URL}/CheckMobileExist?Mobile=${trimmedPhone}`);
    const result = await response.json();
    return result.success
