  ...
  const reviews = [
    {
      ...
      date: 'April 2, 2025',
      rating: 5,
      comment: 'Great doctor, very professional and kind!',
    ...
    },
    {
      ...
      date: 'April 5, 2025',
      rating: 4,
      comment: 'Good experience, would definitely recommend.',
    ...
    },
    {
      ...
      date: 'April 10, 2025',
      rating: 5,
      comment: 'Fantastic service and follow-up care!',
...
const currentReviews = (reviews || []).slice(startIdx, startIdx + itemsPerPage);
    ...
    fetch(
      ...
      .then((data) => {
        if (data.success && Array.isArray(data.data)) {
          setWorkTypes(data.data); // <-- only set the array, not the full response
        }
      })
      .catch((error) => console.error('Failed to fetch work types:', error));
  }, []);
  ...
  useEffect(() => {
    ...
    if (!doctorID) {
      axios
        ...
        .then((res) => {
          if (res.data.success) setDoctorLanguages(res.data.data || []);
        })
      ...
      axios
        ...
        .then((res) => {
          if (res.data.success) setLanguageMaster(res.data.data || []);
        })
  ...
  useEffect(() => {
    ...
    const fetchLOVs = async () => {
      try {
        const [qualRes, specRes] = await Promise.all([
          axios.get(
            'https://predart003-001-site1.anytempurl.com/api/AppLOV?type=Qualification',
          ),
          axios.get(
    ...
    if (!doctorID) {
      axios
        ...
        .catch((err) => console.error('Doctor profile error:', err))
      ...
      axios
        ...
        .then((res) => {
          if (res.data.success) setEducationList(res.data.data || []);
        })
  ...
  useEffect(() => {
    ...
    const fetchEducation = axios.get(
      `https://predart003-001-site1.anytempurl.com/api/Doctor/GetDoctorEducation?doctorId=${doctorID}`,
    );
        ...
        {skills.map((skill) => (
          <li key={skill.skillMasterID}>
            ...
            <p className="text-md text-gray-600">
              Specialization: {getSpecializationName(skill.skillMasterID)}
            </p>
          </li>
        ))}
      </ul>
    ) : (
      <p className="text-md text-gray-500">No skills found for this doctor.</p>
    )}
  </div>
  ...
  <h3 className="text-2xl font-bold text-blue-600 mb-4">Awards</h3>
    {awards.length > 0 ? (
      ...
      <p className="text-md text-gray-500">No awards found for this doctor.</p>
    )}
  </div>
</div>
  ...
  <p className="text-center text-sm text-gray-600 mb-8">
    Based on {reviews.length} Recommendations | Rated 5 Out of 5
  </p>
