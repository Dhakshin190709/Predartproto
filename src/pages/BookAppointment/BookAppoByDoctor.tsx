...
const BookAppoByDoctor = () => {
  ...
  const fetchRelationships = async () => {
    try {
      const response = await fetch("https://predart003-001-site1.anytempurl.com/api/AppLOV?type=Relationship");
...
//   }, [selectedHospitalID, doctors]);
  ...
  useEffect(() => {
    ...
    const handleOptionChange = (selectedOption: AppLOVOption) => {
      ...
      console.log(`Selected: ${selectedOption.name}, appLOVID: ${selectedOption.appLOVID}`);
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.values(newErrors).every((error) => error === '')) {
      ...
      try {
        const response = await fetch('https://predart003-001-site1.anytempurl.com/api/Appointment', {
          method: 'POST',
          headers: {
  ...
  const handleDoctorChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
      ...
      const matchedTimeSlots = data.filter((slot) => String(slot.doctorID) === doctorID);
  ...
  const handleDateChange = (date: Date | null, slots?: TimeSlotType[]) => {
    ...
    const matchedDaySlots = timeSlots.filter(
      (slot) => slot.day?.toLowerCase().trim() === dayOfWeek.toLowerCase().trim()
    );
  
    if (matchedDaySlots.length) {
      matchedDaySlots.forEach(({ fromTime, toTime, slotDuration, timeSlotID }) => {
        generateTimeSlots(fromTime, toTime, slotDuration, timeSlotID);
      });
      setFormData((prev) => ({ ...prev, timeSlotID: matchedDaySlots[0].timeSlotID }));
    } else {
  ...
  useEffect(() => {
    const selectedOption = options.find((opt) => opt.appLOVID === appointmentType);
 ...
 {options.map((option) => (
  <label key={option.appLOVID} className="flex items-center space-x-2">
    <input
      type="radio"
    ...
    <span>{option.name}</span> {/* Display name, but store ID */}
      ...
      />
       {errors.phoneNumber && (
                  ...
                  {appointmentType === options.find((opt) => opt.name === 'Others')?.appLOVID && (
  <div className="mb-4">
      ...
      >
        ...
        {relationships.length > 0 ? (
          relationships.map((relation) => (
            <option key={relation.appLOVID} value={relation.appLOVID}>
              {relation.name}
            </option>
          ))
        ) : (
  ...
  onChange={(time) => {
    if (time) {
      const matchedSlot = generatedTimeSlots.find(
        (slot) => slot.time.getHours() === time.getHours() && slot.time.getMinutes() === time.getMinutes()
      );
