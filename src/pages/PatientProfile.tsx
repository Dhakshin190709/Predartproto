...
const [address, setAddress] = useState({
    ...
    if (result.success && result.data) {
      ...
      setBasicInfo({
       ...
       dob: patientDateOfBirth?.split("T")[0],
        ...
        email: patientEmail,
        gender: patientGender, // Set the gender as received
      });
    }
  })
  .catch((error) => {
  ...
  .then((result) => {
    if (result.success && result.data?.length > 0) {
      setMedicalInfo(result.data[0]); // Access first item in data array
    }
  })
  .catch((error) => {
  ...
  .then((result) => {
    if (result.success && Array.isArray(result.data)) {
      ...
      setFamily(uniqueFamily); // ✅ only unique entries
    }
  })
  .catch((error) => {
