...
const initialData: RowData[] = [
  {
    ...
    description: 'Survey to gather customer satisfaction insights.',
  },
  {
    id: 2,
    ...
    description: 'Internal survey to measure employee engagement levels.',
  },
  {
    id: 3,
    ...
    description: 'Survey for product improvement feedback.',
  },
];

const SurveyMaster: React.FC = () => {
  ...
  const [filterSurveyTitle, setFilterSurveyTitle] = useState('');
  ...
  const handleFilterSearch = () => {
    const filtered = initialData.filter((item) => {
      const matchesTitle = filterSurveyTitle ? item.surveyTitle.toLowerCase().includes(filterSurveyTitle.toLowerCase()) : true;
      //const matchesDescription = filterDescription ? item.description.toLowerCase().includes(filterDescription.toLowerCase()) : true;
      const matchesCreationDate = filterCreationDate ? new Date(item.creationDate) >= new Date(filterCreationDate) : true;
      const matchesExpirationDate = filterExpirationDate ? new Date(item.expirationDate) <= new Date(filterExpirationDate) : true;

      return matchesTitle && matchesDescription && matchesCreationDate && matchesExpirationDate;
    });
  ...
  const applyGlobalSearch = (data: RowData[]) => {
    return data.filter(
      (row) =>
        row.surveyTitle.toLowerCase().includes(quickSearchText.toLowerCase()) ||
        row.description.toLowerCase().includes(quickSearchText.toLowerCase()) ||
        row.creationDate.toLowerCase().includes(quickSearchText.toLowerCase()) ||
        row.expirationDate.toLowerCase().includes(quickSearchText.toLowerCase())
    );
  };

  return (
    <div className="w-full p-4 sm:p-8 xl:p-12 bg-white">
      <h1 className="text-2xl font-semibold text-gray-800 mb-6">Survey Master</h1>

      {/* Filters Section */}
      <div className="flex flex-col gap-4 mb-4">
        {/* First Line: Survey Title, Creation Date, Expiration Date */}
        <div className="flex gap-4">
          <input
            type="text"
            placeholder="Survey Title"
            value={filterSurveyTitle}
            onChange={(e) => setFilterSurveyTitle(e.target.value)}
          ...
          <input
            ...
            onBlur={(e) => (e.target.type = filterCreationDate ? 'date' : 'text')}
      ...
      <div className="mb-4 mt-4 flex flex-wrap gap-4 justify-between items-center">
        <div className="relative">
          <input
            ...
            className="sm:w-60 w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 text-black outline-none focus:border-primary"
          />
